...
from config import CARRIERS_FILE, ORDERS_FILE, PRODUCTS_FILE
...
def shipping_api_tool(request: str) -> str:
    ...
    try:
        ...
        result = build_shipping_plan(limit=limit, optimization_mode=mode)
        ...
        for item in result["shipping_plan"]:
            output.append(
                ...
                f"Quantity: {item['qty']}\n"
                f"Region: {item['ship_to_region']}\n"
                f"Chosen Carrier: {item['chosen_carrier']}\n"
                f"Service Level: {item['service_level']}\n"
                f"Shipping Cost: {item['shipping_cost']}\n"
                f"ETA Days: {item['eta_days']}\n"
...
""",
    ...
    logistics_crew = Crew(
        agents=[logistics_routing_agent],
...
if __name__ == "__main__":
    ...
    print("\n" + "=" * 70 + "\nFINAL LOGISTICS AGENT RESPONSE\n" + "=" * 70 + f"\n{result}")
